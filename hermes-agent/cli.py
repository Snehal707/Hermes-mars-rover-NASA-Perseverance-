...
class HermesCLI:
    ...
    def _handle_sendfile_command(self, cmd: str):
        """Handle /sendfile command for direct file delivery to messaging targets."""
        try:
            parts = shlex.split(cmd)
        except ValueError as e:
            print(f"(._.) Could not parse command: {e}")
            print('  Usage: /sendfile [target] <file_path> [caption]')
            return
        ...
        def _looks_like_path(token: str) -> bool:
            ...
            if token.startswith(("/", "~", ".")):
                return True
            return len(token) >= 2 and token[1] == ":"  # Windows drive path, e.g. C:\...

        if len(parts) < 2:
            print("(._.) Usage: /sendfile [target] <file_path> [caption]")
            print("  Example: /sendfile /tmp/report.pdf")
            print('  Example: /sendfile telegram /tmp/report.pdf "Automation report"')
            print("  Default target is telegram if omitted.")
            return
        ...
        if len(parts) == 2:
            ...
            if not _looks_like_path(parts[1]):
                print("(._.) Missing file path.")
                print("  Usage: /sendfile [target] <file_path> [caption]")
                print("  Example: /sendfile telegram /tmp/report.pdf")
                return
            file_token = parts[1]
        else:
            # /sendfile telegram /path/to/file.pdf [caption]
            # /sendfile /path/to/file.pdf [caption]
            if _looks_like_path(parts[1]):
                ...
                target = parts[1]
                file_token = parts[2]
                caption = " ".join(parts[3:]).strip() if len(parts) > 3 else ""

        file_path = Path(file_token).expanduser()
        if not file_path.is_absolute():
            file_path = Path(os.getcwd()) / file_path
        file_path = file_path.resolve()

        if not file_path.exists():
            print(f"(._.) File not found: {file_path}")
            return
        if not file_path.is_file():
            print(f"(._.) Not a file: {file_path}")
            return

        media_line = f"MEDIA:{file_path}"
        outbound = f"{caption}\n{media_line}".strip() if caption else media_line
        ...
        try:
            ...
            print(f"(x_x) Failed to send file: {e}")
            return

        if result.get("error"):
            print(f"(x_x) Send failed: {result['error']}")
            if "home channel" in str(result["error"]).lower():
                print("  Tip: run /platforms and set TELEGRAM_HOME_CHANNEL if needed.")
            return

        msg_ids = result.get("message_ids") or []
        single_id = result.get("message_id")
        if single_id:
            msg_ids.append(single_id)
        print(f"(^_^)b File sent to {target}: {file_path}")
        if msg_ids:
            print(f"  Message IDs: {', '.join(str(m) for m in msg_ids)}")
    ...
    def process_command(self, command: str) -> bool:
        ...
        elif cmd_lower.startswith("/sendfile"):
            self._handle_sendfile_command(cmd_original)
