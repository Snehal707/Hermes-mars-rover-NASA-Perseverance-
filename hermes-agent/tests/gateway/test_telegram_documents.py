...
# ---------------------------------------------------------------------------
# Outbound sending (native file delivery)
# ---------------------------------------------------------------------------

class TestOutboundNativeFiles:
    @pytest.mark.asyncio
    async def test_send_document_native_success(self, adapter, tmp_path):
        ...
        result = await adapter.send_document(
            ...
            file_name="mars_rover_report.pdf",
            reply_to="42",
        )
        ...
        assert kwargs["filename"] == "mars_rover_report.pdf"
        assert kwargs["caption"] == "Report ready"
        assert kwargs["reply_to_message_id"] == 42

    @pytest.mark.asyncio
    async def test_send_document_missing_file(self, adapter, tmp_path):
        adapter._bot = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_send_video_native_success(self, adapter, tmp_path):
        file_path = tmp_path / "clip.mp4"
        file_path.write_bytes(b"\x00\x00\x00\x20ftypisom")

        sent = MagicMock()
        sent.message_id = 777
        adapter._bot = AsyncMock()
        adapter._bot.send_video = AsyncMock(return_value=sent)
        ...
        assert kwargs["chat_id"] == 100
        assert kwargs["caption"] == "Rover clip"
