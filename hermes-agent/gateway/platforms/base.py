...
def _media_debug(msg: str, *args, **kwargs) -> None:
    if _media_debug_enabled():
        logger.warning("[media-debug/adapter] " + msg, *args, **kwargs)
...
class BasePlatformAdapter(ABC):
    ...
    def extract_media(content: str) -> Tuple[List[Tuple[str, bool]], str]:
        ...
        if media:
            ...
            cleaned = re.sub(r'\n{3,}', '\n\n', cleaned).strip()
        _media_debug("extract_media found=%d paths=%s", len(media), [p for p, _ in media])
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                _media_debug(
                    "process_response chat_id=%s media_files=%s text_len=%d",
                    event.source.chat_id if event.source else "<none>",
                ...
                for media_path, is_voice in media_files:
                    ...
                    try:
                        ...
                        _media_debug(
                            "send_media chat_id=%s path=%s ext=%s exists=%s is_voice=%s",
                            event.source.chat_id if event.source else "<none>",
                            media_path,
                            ext,
                            Path(media_path).is_file(),
                            is_voice,
                        )
                        ...
                        if not media_result.success:
                            _media_debug(
                                "send_media_failed chat_id=%s path=%s ext=%s error=%s",
                                event.source.chat_id if event.source else "<none>",
                                media_path,
                                ext,
                                media_result.error,
                        ...
                        _media_debug(
                            "send_media_exception chat_id=%s path=%s error=%s",
                            event.source.chat_id if event.source else "<none>",
                            media_path,
                            media_err,
                            exc_info=True,
