...
def _media_debug(msg: str, *args, **kwargs) -> None:
    if _media_debug_enabled():
        logger.warning("[media-debug/gateway] " + msg, *args, **kwargs)
...
class GatewayRunner:
    ...
    @staticmethod
    def _wants_report_pdf_request(text: str) -> bool:
        """True when a message asks for a report in PDF form."""
        if not text:
            return False
        lowered = text.lower().strip()
        return "pdf" in lowered and ("report" in lowered or "summary" in lowered)

    @staticmethod
    def _get_api_base_url() -> str:
        """Return API base URL used for report endpoints."""
        return (os.getenv("API_URL") or "http://localhost:8000").rstrip("/")

    @staticmethod
    def _get_api_headers() -> Dict[str, str]:
        """Build optional auth headers for calls to the Mars Rover API."""
        headers: Dict[str, str] = {}
        api_key = (os.getenv("ROVER_API_KEY") or "").strip()
        if api_key:
            headers["X-API-Key"] = api_key
        return headers

    async def _fetch_report_pdf_bytes(self) -> Optional[bytes]:
        """Fetch PDF bytes from API_URL/report/pdf."""
        url = f"{self._get_api_base_url()}/report/pdf"
        try:
            import httpx
            ...
            logger.debug("Report PDF request failed: %s", e)
        return None

    async def _fetch_report_text(self) -> Optional[str]:
        """Fetch text report from API_URL/report via GET or POST."""
        url = f"{self._get_api_base_url()}/report"
        try:
            import httpx
            async with httpx.AsyncClient(timeout=30.0) as client:
                for method in ("GET", "POST"):
                    try:
                        response = await client.request(method, url, headers=self._get_api_headers())
                    except Exception:
                        continue
                    if response.status_code == 200:
            ...
            logger.debug("Report text request failed: %s", e)
        return None

    async def _handle_report_pdf_shortcut(self, event: MessageEvent) -> Optional[str]:
        """
        Handle "report in pdf" requests at the gateway layer.

        This bypasses agent forwarding and returns a MEDIA tag so platform
        adapters can send a native document attachment.
        """
        if not self._wants_report_pdf_request(event.text):
            return None

        pdf_bytes = await self._fetch_report_pdf_bytes()
        if pdf_bytes:
            try:
                cached_path = cache_document_from_bytes(pdf_bytes, "mars_rover_report.pdf")
            except Exception as e:
                logger.warning("Could not cache report PDF from API: %s", e)
            else:
                return (
                    "Here is your Mars Rover Research Report in PDF format.\n"
                    f"MEDIA:{cached_path}"
                )

        report_text = await self._fetch_report_text()
        if report_text:
            return (
                "PDF is not available from the server right now. "
                "Here is the text report:\n\n"
                + report_text[:3500]
            )

        return (
            "PDF is not available from the server right now, and the report API "
            "is unreachable."
        )

    async def _handle_report_pdf_shortcut_text(self, text: str) -> Optional[str]:
        """
        Handle report-PDF requests from already-enriched text (e.g. voice STT).

        This lets voice commands like "send report in PDF" hit the same API
        shortcut path instead of being forwarded to the agent.
        """
        if not self._wants_report_pdf_request(text):
            return None
        temp_event = MessageEvent(text=text, message_type=MessageType.TEXT, source=None)
        return await self._handle_report_pdf_shortcut(temp_event)

    @staticmethod
    def _extract_media_paths(text: str) -> list[str]:
        """Extract MEDIA file paths from text with tolerant parsing."""
        if not text or "MEDIA:" not in text:
            return []
        paths: list[str] = []
        pattern = r'(?i)\bMEDIA\s*:\s*(?:"([^"]+)"|\'([^\']+)\'|([^\n\r]+))'
        for match in re.finditer(pattern, text):
            raw = (match.group(1) or match.group(2) or match.group(3) or "").strip()
            raw = raw.strip("`").rstrip('",}])')
            if raw:
                paths.append(raw)
        _media_debug("extract_media_paths found=%d paths=%s", len(paths), paths)
        return paths

    @staticmethod
    def _cache_media_file(path: str) -> str:
        """
        Copy MEDIA files to persistent cache so /tmp paths don't disappear
        before adapters upload them.
        """
        try:
            p = Path(path)
            # Briefly wait for files that are still being flushed to disk.
            last_size = None
            for _ in range(15):
            ...
            if not p.exists() or not p.is_file():
                _media_debug("cache_media_file missing path=%s", path)
                return path
            cached = cache_document_from_bytes(p.read_bytes(), p.name)
            _media_debug("cache_media_file path=%s cached=%s", path, cached or path)
            return cached or path
        except Exception:
            _media_debug("cache_media_file exception path=%s", path, exc_info=True)
            return path

    def _rewrite_media_tags_with_cached_paths(self, text: str) -> str:
        """Rewrite MEDIA:<path> tags to persistent cached paths when possible."""
        if not text or "MEDIA:" not in text:
            return text

        def _replace(match: re.Match) -> str:
            raw_path = (match.group(1) or match.group(2) or match.group(3) or "").strip()
            raw_path = raw_path.strip("`").rstrip('",}])')
            if not raw_path:
                return match.group(0)
            cached_path = self._cache_media_file(raw_path)
            _media_debug("rewrite_media_tag raw=%s cached=%s", raw_path, cached_path)
            return f"MEDIA:{cached_path}"
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        # Shortcut: handle "report in pdf" directly in gateway before agent run.
        report_shortcut = await self._handle_report_pdf_shortcut(event)
        if report_shortcut is not None:
        ...
        # If transcription/caption now clearly asks for report PDF, bypass agent.
        # This covers voice/audio flows where event.text was initially empty.
        report_shortcut_after_enrich = await self._handle_report_pdf_shortcut_text(message_text or "")
        if report_shortcut_after_enrich is not None:
            return report_shortcut_after_enrich
    ...
    async def _run_agent(
        ...
        def run_sync():
            ...
            max_tokens = None
            max_tokens_raw = (os.getenv("HERMES_MAX_TOKENS") or "").strip()
            if max_tokens_raw:
                try:
                    ...
                    if parsed_max_tokens > 0:
                        ...
                        logger.warning(
                            "Ignoring HERMES_MAX_TOKENS=%r (must be > 0).",
                            max_tokens_raw,
                        )
                except ValueError:
                    logger.warning(
                        "Ignoring HERMES_MAX_TOKENS=%r (must be an integer).",
                        max_tokens_raw,
                    )
            ...
            for _hm in agent_history:
                if _hm.get("role") in ("tool", "function"):
                    _hc = _hm.get("content", "")
                    if "MEDIA:" in _hc:
                        for _p in self._extract_media_paths(_hc):
            ...
            # If the model already returned MEDIA tags, rewrite them to cached
            # paths immediately so temporary files don't disappear before upload.
            if "MEDIA:" in final_response:
                _media_debug("final_response has MEDIA before rewrite")
                final_response = self._rewrite_media_tags_with_cached_paths(final_response)
                _media_debug("final_response after rewrite len=%d", len(final_response))
            ...
            if "MEDIA:" not in final_response:
                ...
                for msg in result.get("messages", []):
                    if msg.get("role") in ("tool", "function"):
                        content = msg.get("content", "")
                        if "MEDIA:" in content:
                            for path in self._extract_media_paths(content):
                                if path and path not in _history_media_paths:
                                    cached_path = self._cache_media_file(path)
                                    media_tags.append(f"MEDIA:{cached_path}")
                            if "[[audio_as_voice]]" in content:
                                has_voice_directive = True
                
                if media_tags:
                    _media_debug("collected media_tags=%s", media_tags)
                    seen = set()
                    unique_tags = []
